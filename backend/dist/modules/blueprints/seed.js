...
exports.seedDefaultBlueprints = seedDefaultBlueprints;
const bcrypt_1 = __importDefault(require("bcrypt"));
const User_1 = require("../../db/models/User");
const service_1 = require("./service");
async function ensureSeedUser() {
    const email = process.env.SEED_DEFAULT_EMAIL ||
    ...
    const [user] = await User_1.User.findOrCreate({
        ...
        defaults: { email: String(email), passwordHash }
    });
...
async function seedDefaultBlueprints() {
    ...
    await (0, service_1.createBlueprint)({
        userId: seedUser.id,
    ...
    if (!shouldSeedSupport)
        return;
    const passwordHash = await bcrypt_1.default.hash(String(supportPassword), 12);
    const [supportUser] = await User_1.User.findOrCreate({
        where: { email: String(supportEmail) },
        defaults: { email: String(supportEmail), passwordHash }
    });
